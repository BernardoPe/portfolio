...
export default function ProjectCard({ project }: Props): React.JSX.Element {
  return (
    <div className="bg-secondary rounded-xl px-8 py-6 shadow-lg w-full h-full flex flex-col max-w-4xl">
      <div className="flex-1">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-4">
            <h3 className="text-2xl md:text-3xl font-bold color-primary">{project.title}</h3>
            <a
              ...
              className="inline-flex items-center justify-center color-secondary opacity-80 hover:opacity-100 hover:color-primary transition-all duration-200"
            >
        ...
        <p className="color-secondary text-sm md:text-base font-medium mb-3">{project.subtitle}</p>

        {project.role && (
          <div className="inline-block mb-4">
            <span className="bg-slate-700/70 color-primary px-3 py-1 rounded text-sm">
              {project.role}
            </span>
          </div>
        )}

        <p className="text-base color-tertiary mb-6 text-sm md:text-base leading-relaxed">
          {project.description}
        </p>
        <div className="mb-6">
          <div
          ...
          >
            {project.technologies.map((tech, index) => (
              <div key={index} className="flex flex-col items-center gap-1.5">
                <div className="w-10 h-10 flex items-center justify-center bg-slate-700/10 rounded-full">
                  {tech.icon ? (
                    <img src={tech.icon} alt={tech.name} className="w-8 h-8 object-contain" />
                  ) : (
                    <span className="w-8 h-8 flex items-center justify-center text-xs font-semibold color-primary">
                      {tech.name.charAt(0).toUpperCase()}
                    </span>
                  )}
                </div>
                <span className="text-xs md:text-sm color-secondary text-center max-w-[5rem] break-words">
                  {tech.name}
                </span>
              </div>
            ))}
